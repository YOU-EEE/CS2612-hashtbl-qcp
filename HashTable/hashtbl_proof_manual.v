...
Lemma store_name_intro : forall p key_ptr k,
  (&(p # "blist" ->ₛ "key")) # Ptr |-> key_ptr ** store_string key_ptr k
  |-- store_name k p.
Proof.
  intros.
  unfold store_name.
...
Lemma store_hash_skeleton_intro :
  forall (x: addr) (m: list Z -> option addr)
    ...
    (&(x # "hashtbl" ->ₛ "bucks") # Ptr |-> bucks ** PtrArray.full bucks 211 lh) **
    store_map store_sll b ** store_map store_name m
    |-- store_hash_skeleton x m.
Proof.
  pre_process.
  unfold store_hash_skeleton.
...
Proof.
  pre_process.
  unfold store_hash_skeleton.
  Exists nil (zeros 211) empty_map 0 bucks_base.
  setoid_rewrite (store_map_empty store_sll).
