...
(** 这段证明有三个证明步骤。证明指令_[intros]_将待证明结论中的逻辑结构“对于任意整数x与
    y”移除，并在前提中假如“x与y是整数”这两条假设。第二条指令_[pose proof]_表示对
    _[x - y]_使用标准库中的定理_[sqr_pos]_。从Coq定理证明界面中可以看到，使用该定理得
    到的命题会被添加到证明目标的前提中去，Coq系统将这个新命题自动命名为H（表示
    Hypothesis）。最后，_[nia]_可以自动根据当前证明目标中的前提证明结论。

    在Coq中，可以反复使用_[pose proof]_指令，进而得到一系列之后可以使用的数学命题。例
    如，_[sum_of_sqr2]_的证明中，如果只使用_[lia]_那就需要手动引入更多的代数变化步骤
    了，具体而言，下面我们讲额外用到整数乘法对整数减法的左右分配律。*)

Fact sum_of_sqr2_lia_only: forall x y: Z,
  x * x + y * y >= 2 * x * y.
Proof.
  intros.
  pose proof sqr_pos (x - y).
  pose proof Z.mul_sub_distr_r x y (x - y).
  pose proof Z.mul_sub_distr_l x x y.
  pose proof Z.mul_sub_distr_l y x y.
  lia.
Qed.

(** 这段证明中额外增加的三条_[pose proof]_指令会生成下面命题：
    ...
    Coq中的_[pose proof]_也可以引用我们自己证明的结论。下面证明演示了如何使用我们刚刚
    证明的性质_[sum_of_sqr1]_。*)
...
Fact logic_ex1: forall f: Z -> Z,
  ...
  (forall n, nonneg n /\ Z.Even n ->
             nonneg (f n) /\ Z.Even (f n)).
